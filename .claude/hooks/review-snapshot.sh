...
BASELINE_FILE=".claude/reviews/baseline.json"
BASELINE_SHA=""

if [[ -f "$BASELINE_FILE" ]]; then
    if command -v jq >/dev/null 2>&1; then
        BASELINE_SHA=$(jq -r '.sha // empty' "$BASELINE_FILE" 2>/dev/null)
    else
        BASELINE_SHA=$(grep -o '"sha"[[:space:]]*:[[:space:]]*"[^"]*"' "$BASELINE_FILE" 2>/dev/null \
...
(
    ...
    if make -n test >/dev/null 2>&1; then
        TEST_AVAILABLE=true
        # TODO: quality-gate.sh also runs tests — dedup in a later iteration
