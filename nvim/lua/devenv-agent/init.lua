...
---@return boolean true if send was dispatched
local function send_message(user_input)
	...
	streaming = true
	local full_response = ""
	local generation_at_send = session_generation
	...
	provider.send(conversation, system, function(chunk)
		if generation_at_send ~= session_generation then
			return
		end
...
local function initial_content()
	...
	return {
		...
		summary,
		"",
		"Type below. **Ctrl-Enter** / **Ctrl-s** to send (**Enter** in normal mode). **Tab**(n) / **S-Tab**(i) switch panes. **q**(n) / **Ctrl-c** close.",
		"",
		"---",
	}
end
...
--- Read input buffer, send message, clear input buffer only on success
local function send_from_input()
	...
	if send_message(text) then
		-- Clear input buffer only after successful dispatch
...
local function setup_keybindings()
	...
	input_popup:map("n", "q", teardown_layout, map_opts)
...
local function setup_win_closed()
	...
	vim.api.nvim_create_autocmd("WinClosed", {
		...
		callback = function(args)
			...
			if (history_popup and closed == history_popup.winid) or (input_popup and closed == input_popup.winid) then
				teardown_layout()
			end
		end,
	})
	...
	input_popup = Popup({
		...
		border = {
			...
			text = {
				top = " Input ",
				top_align = "left",
				bottom = " <C-CR>/<C-s> send | <CR>(n) send | <Tab>(n)/<S-Tab>(i) pane | <C-c> close ",
				bottom_align = "center",
			},
		},
		buf_options = {
